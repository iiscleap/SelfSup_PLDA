...
class PIC_dihard_threshold:
    ...
    def gacBuildDigraph(self):
        ...
        self.A[np.diag_indices(self.N)] = -np.inf
        ...
        # build graph
        # ND = 1/(1+np.exp(sortedDist[:, 1:self.K+1]))
    ...
    def gacNNMerge(self,NNIndex):
        ...
        if len(self.clusterlen)== self.N:
            ...
            self.labelfull = np.squeeze(clusterLabels)
        else:
            clusterLabels=self.labelfull.reshape(-1,1)
    ...
    def gacMerging_oracle_org(self,graphW, initClusters):
    ...
    def gacCluster_oracle_org(self,init=0):
        """
        Input: 
        ...
        # graphW, NNIndex = self.gacBuildDigraph_with_threshold(0.5)
        ...
        if not init:
            print('--------------------------  merging --------------------------')
            self.gacMerging_oracle_org(graphW, initialClusters)
        
        return self.labelfull,self.clusterlen

    def gacMerging_oracle(self,graphW, initClusters):
        # % Cluster merging for Graph Agglomerative Clustering 
        # Implements an agglomerative clustering algorithm based on maiximum graph
        # strcutural affinity of two groups
        # Inputs:
        ...
        if numClusters > groupNumber:
          while True: 
            ...
            if curGroupNum <= 20: 
                ...
                min_len = min(20,round(numSample*0.1))
                # min_len = 10
                ...
                if curGroupNum == groupNumber+1:                    
                    num_with_min_cardinality = len(len_initclusters[len_initclusters >= min_len])
            ...
            # % update the affinity table for the merged cluster
            clusterComp[minIndex1] = complexity_fun(IminuszW[np.ix_(initClusters[minIndex1], initClusters[minIndex1])])
            for groupIndex1 in  range(minIndex1):
                affinityTab[groupIndex1, minIndex1] = - conditionalComplexity_fun(IminuszW, initClusters[groupIndex1], initClusters[minIndex1])
            
            for groupIndex1 in  range(minIndex1+1,curGroupNum):
                affinityTab[minIndex1, groupIndex1] = - conditionalComplexity_fun(IminuszW, initClusters[groupIndex1], initClusters[minIndex1])
            
            affinityTab[:minIndex1, minIndex1] = clusterComp[:minIndex1,0] + clusterComp[minIndex1,0] + affinityTab[:minIndex1, minIndex1]
            affinityTab[minIndex1, minIndex1+1:curGroupNum] = clusterComp[minIndex1+1:curGroupNum,0] + clusterComp[minIndex1,0] + affinityTab[minIndex1, minIndex1+1:curGroupNum]
        else:
            groupNumber = numClusters
        # find overlapping segments using weights W of each sample to each cluster
    ...
    def gacCluster_oracle(self):
        ...
        print('--------------------------  merging --------------------------')
        self.gacMerging_oracle(graphW, initialClusters)
        ...
        while True: 
            ...
            if curGroupNum <= 20: 
                ...
                if np.sum(affinityTab[iu1])==0:
                    break
                if curGroupNum == min(20,numClusters):
    ...
    def gacMerging_org(self,graphW, initClusters):
        # % Cluster merging for Graph Agglomerative Clustering 
        # Implements an agglomerative clustering algorithm based on maiximum graph
        # strcutural affinity of two groups
        # Inputs:
        ...
        th = self.th
        print('intial clusters:',numClusters)
        max_spks=10
        while True: 
            # if (curGroupNum % 20 ) == 0 and VERBOSE:
            #     print('Group count: %d' % curGroupNum)
    ...
    def gacCluster_org(self):
        ...
        print('--------------------------  merging --------------------------')
        self.gacMerging_org(graphW, initialClusters)
...
class PIC_ami_threshold:
    ...
    def gacCluster_oracle_org(self):
        """
        Input: 
        - A: pairwise distances, d_{i -> j}
        - groupNumber: the final number of clusters
